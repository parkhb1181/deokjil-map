 ...
 * **서버가 있으면 서버, 없으면 목데이터.**
 *
 * 서버 목록에는 아직 **마지막 메시지가 안 온다** (CH-05 — chat.ts 머리말).
 * 그래서 API 경로는 그 줄이 비고, 정렬도 약속 시각 순이다. 안 읽은 수는
 * 서버가 주기 시작하면(STAR-117) 그대로 배지가 된다. 목데이터는 두 줄을
 * 그린 채 두어 서버가 채웠을 때의 모양을 팀이 볼 수 있게 한다.
 *
