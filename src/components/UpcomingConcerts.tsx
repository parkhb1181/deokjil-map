 ...
 * **48번째 카드**에서 처음 나온다 (2026-09-09 실측). 상단 순위 다섯도
 * 전부 생카 아티스트다. 그래서 「콘서트가 아예 없다」 로 읽힌다.
 *
 ...
 *
 * **포스터를 안 건다.** KOPIS 포스터는 우리가 크기를 못 줄이는 주소라
 * (`poster.ts` 가 offmate 만 줄인다) 열넷을 걸면 이미지 최적화 비용이
 * 그대로 붙는다. 위의 순위 블록이 이미 사진을 크게 쓰고 있어서, 여기서
 * 또 깔면 첫 화면이 사진 두 덩어리가 된다.
 *
  ...
  return (
    <section className="upnext">
      <div className="upnext__head">
        <h2 className="upnext__title">곧 열리는 콘서트</h2>
        {/* 「모두」 는 종류 필터를 옮기는 것이다. 다른 화면으로 보내지
            않는다 — 돌아올 길을 따로 만들어야 하고, 콘서트만 보고 싶은
            사람이 쓰려던 것이 원래 그 필터다 */}
