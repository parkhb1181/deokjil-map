 ...
 *
 *   /              실서비스. 진짜 방문자가 온다
 *   /w4871/home    온보딩에서 팀이 누르는 자리
 *
 * 다른 것은 `wireframe` 하나뿐이다. 로그인 버튼과 하단 탭의 동행이
 * 그 값으로 갈린다. 둘 다 아직 와이어프레임 화면으로 이어지므로
 * 실서비스에서는 그리면 안 되고, 팀은 봐야 한다.
 *
 * 복사해서 둘로 두지 않는 이유는 한쪽만 고쳐지면 팀이 보는 화면과
 * 실제로 나가는 화면이 갈라지기 때문이다. 갈라진 것을 알아챌 방법도 없다.
 *
 * 빌드 플래그로 가르지 않는 이유도 같다. duckmoim.com 은 와이어프레임
      ...
      <div className="app">
        <header className="header">
          <div className="header__row">
            ...
            {/* 로고만 남아 비어 있던 오른쪽에 로그인을 둔다.

                이 자리를 채울 것을 한참 찾았는데, 즐겨찾기는 하단 탭에
                있고 건수는 바로 아래 순위 섹션에 있어 갈 곳이 없었다.
                로그인은 다르다. 비회원으로 둘러보다 댓글을 쓰려는 순간
