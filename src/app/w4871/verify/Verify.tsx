 ...
 * 달라고만 하면 사람들은 안 준다.
 *
 * **받은 번호로 무엇을 하는지는 백엔드 명세가 정했다** (위키 2차
 * 명세서 PV-04 · PV-05 · PV-07 · PV-08).
 *
 *   원본을 저장하지 않는다. 되돌릴 수 없는 값으로 바꿔서만 둔다
 *   한 번호로 쓸 수 있는 계정은 하나다
 *   탈퇴하면 지운다. 제재 이력이 있을 때만 남긴다
 *   인증했다는 표시를 남에게 보이지 않는다 — 신원 보증으로 읽힌다
 *
 * 화면의 안내 문구가 이 넷과 어긋나면 안 된다. 처리방침 제1조·제3조와
 * 약관 제4조가 같은 말을 하고 있다.
 *
    ...
    <PageShell title="휴대전화 인증">
      <div className="vfy">
        ...
        <p className="vfy__lead">
          모집글과 댓글, 채팅을 쓰려면 번호 확인이 한 번 필요해요.
          <br />
          <b>번호는 다른 사람에게 보이지 않고, 저장할 때도 알아볼 수 없게 바꿔서 둡니다.</b>
        </p>
        ...
        <p className="vfy__why">
          모르는 사람과 직접 만나는 서비스라 한 번 확인합니다. 한 번호로 계정 하나만 쓸 수
          있게 해서, 이용 제한을 받고 다시 들어오는 것을 막는 데 씁니다. 번호 자체는 저장하지
          않고 알아볼 수 없는 값으로 바꿔 두며, 인증했다는 표시도 프로필에 달지 않습니다.
        </p>
      </div>
    </PageShell>
  )
