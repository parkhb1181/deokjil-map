 ...
 * 해당하지 않고(`run.mjs` 가 이 소스만 게이트를 건너뛴다), 이용제한에서 호출량에
 * 걸리는 것은 **초당 10회**뿐이다 — 일일 건수 한도는 없다(2026-09-08 확인).
 * lib/http.mjs 의 700ms 간격이 세 소스를 통틀어 직렬화해 최대 1.43회/초다.
 *
 ...
 * 전국 전 장르를 받아서 버리면 상세 호출이 그만큼 늘어난다 — 목록 1건당 상세 1회고,
 * 700ms 간격이 곱해져 잡 실행 시간이 그만큼 길어진다.
 *
